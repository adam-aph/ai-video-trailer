...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/cinecut/cli.py"
      provides: "Stage 5 structural analysis gate; TOTAL_STAGES=8; duration extraction in Stage 1; structural_anchors passed to run_narrative_stage at Stage 6"
      contains: "Stage 5"
    - path: "src/cinecut/narrative/generator.py"
      provides: "run_narrative_stage accepts structural_anchors kwarg and passes it to TrailerManifest"
      contains: "structural_anchors"
  ...
  key_links:
    ...
    - from: "src/cinecut/cli.py Stage 6"
      to: "src/cinecut/narrative/generator.py run_narrative_stage"
      via: "run_narrative_stage(..., structural_anchors=structural_anchors) — variable in scope from Stage 5"
      pattern: "structural_anchors=structural_anchors"
    - from: "src/cinecut/narrative/generator.py run_narrative_stage"
      to: "src/cinecut/manifest/schema.py TrailerManifest"
      via: "TrailerManifest(..., structural_anchors=structural_anchors) — kwarg passed through from caller"
      pattern: "structural_anchors=structural_anchors"
---

<objective>
Implement structural analysis: subtitle chunking + Mistral inference + heuristic fallback + manifest v2.0 schema bump + Stage 5 in cli.py + generator.py wiring so structural_anchors reaches the manifest.

Purpose: This plan completes Phase 7. After 07-01 provides TextEngine and get_models_dir, this plan adds the analysis logic (structural.py), the analyze_chunk method on TextEngine, the StructuralAnchors model in schema.py, checkpoint fields, and wires everything into cli.py as a new Stage 5. The manifest schema bumps from "1.0" to "2.0" with the structural_anchors block. generator.py is updated so run_narrative_stage forwards structural_anchors to the TrailerManifest constructor — without this step, model_dump_json(exclude_none=True) would silently omit the block.
...
- src/cinecut/cli.py (Stage 5 inserted, TOTAL_STAGES=8, duration captured in Stage 1, structural_anchors passed to Stage 6 call)
- src/cinecut/narrative/generator.py (run_narrative_stage accepts and forwards structural_anchors kwarg)
...
From src/cinecut/narrative/generator.py (current run_narrative_stage signature — to be extended):
```python
def run_narrative_stage(
    ...
    ...
    # Near end of function, current TrailerManifest instantiation:
    ...
    output_path.write_text(
        manifest.model_dump_json(indent=2, exclude_none=True),  # exclude_none=True silently drops structural_anchors=None
        encoding="utf-8",
    )
```
...
# Current run_narrative_stage call site (becomes Stage 6 after renumbering):
...
# structural_anchors variable is in scope from Stage 5 — must be passed here
```
</interfaces>
</context>
...
<task type="auto">
  <name>Task 2: Add StructuralAnchors to manifest/schema.py, extend checkpoint.py, wire Stage 5 in cli.py, update generator.py</name>
  <files>
    src/cinecut/manifest/schema.py
    src/cinecut/checkpoint.py
    src/cinecut/cli.py
...
7. Update the Stage 6 (formerly Stage 5) `run_narrative_stage` call to pass `structural_anchors`:
   ```python
   manifest_path = run_narrative_stage(
       inference_results,
       dialogue_events,
       ...
       work_dir,
       progress_callback=_narr_callback,
       structural_anchors=structural_anchors,   # ADD THIS — in scope from Stage 5
   )
   ```
   The `structural_anchors` variable is always set before Stage 6 executes (either from the Stage 5 if-branch or the else-branch resume path).

**Part D — narrative/generator.py — accept and forward structural_anchors:**

This is the critical wiring step. Without it, `model_dump_json(exclude_none=True)` silently drops the `structural_anchors` block from the written JSON even after schema.py is updated.

1. Add `StructuralAnchors` to the imports at top of generator.py:
   ```python
   from cinecut.manifest.schema import ClipEntry, TrailerManifest, StructuralAnchors
   ```

2. Add `Optional` to the typing imports (if not already present):
   ```python
   from typing import Callable, Optional
   ```

3. Add `structural_anchors` parameter to `run_narrative_stage` signature (as the last keyword argument, after `progress_callback`):
   ```python
   def run_narrative_stage(
       inference_results: list,
       dialogue_events: list,
       ...
       work_dir: Path,
       progress_callback: Callable[[int, int], None] | None = None,
       structural_anchors: Optional[StructuralAnchors] = None,   # ADD
   ) -> Path:
   ```

4. Update the `TrailerManifest` instantiation near the end of `run_narrative_stage` (currently 3 kwargs) to pass `structural_anchors`:
   ```python
   manifest = TrailerManifest(
       source_file=str(source_file),
       vibe=vibe,
       clips=clip_entries,
       structural_anchors=structural_anchors,   # ADD — None is fine; field is Optional
   )
   ```
   The `model_dump_json(exclude_none=True)` call that follows will write the block when structural_anchors is not None, and omit it when None (backward-compat for callers that do not provide it).
  </action>
  <verify>
...
# Test manifest JSON includes structural_anchors when populated
j = json.loads(m2.model_dump_json(indent=2, exclude_none=True))
assert 'structural_anchors' in j, 'structural_anchors missing from JSON output'
assert j['structural_anchors']['begin_t'] == 300.0
# Test generator.py run_narrative_stage accepts structural_anchors kwarg
import inspect
from cinecut.narrative.generator import run_narrative_stage
sig = inspect.signature(run_narrative_stage)
assert 'structural_anchors' in sig.parameters, 'run_narrative_stage missing structural_anchors param'
print('All schema/checkpoint/generator assertions passed')
"</automated>
  </verify>
  <done>StructuralAnchors importable from manifest.schema; TrailerManifest.schema_version defaults to "2.0"; structural_anchors field Optional and defaults None; model_dump_json with structural_anchors populated writes the block to JSON; PipelineCheckpoint has proxy_duration_s and structural_anchors fields; run_narrative_stage accepts structural_anchors kwarg and passes it to TrailerManifest; cli.py imports cleanly; TOTAL_STAGES=8</done>
</task>
...
import inspect
sig = inspect.signature(run_narrative_stage)
assert 'structural_anchors' in sig.parameters
print('All imports OK')
print('TOTAL_STAGES:', end=' ')
import cinecut.cli as c; print(c.TOTAL_STAGES)
assert c.TOTAL_STAGES == 8
print('generator.py structural_anchors param: OK')
"
```

Manifest JSON contains structural_anchors block:
```bash
python -c "
import json
from cinecut.manifest.schema import StructuralAnchors, TrailerManifest, ClipEntry
a = StructuralAnchors(begin_t=270.0, escalation_t=2430.0, climax_t=4320.0, source='heuristic')
ce = ClipEntry(source_start_s=0.0, source_end_s=5.0, beat_type='breath', act='act1')
m = TrailerManifest(source_file='film.mkv', vibe='drama', clips=[ce], structural_anchors=a)
j = json.loads(m.model_dump_json(indent=2, exclude_none=True))
assert j['schema_version'] == '2.0'
assert 'structural_anchors' in j
assert j['structural_anchors']['source'] == 'heuristic'
print('Manifest JSON structural_anchors block: OK')
print(json.dumps(j['structural_anchors'], indent=2))
"
```
...
- cli.py: TOTAL_STAGES=8; Stage 5 structural analysis gate present; heuristic fallback when GGUF absent; Stage 6 run_narrative_stage call passes structural_anchors=structural_anchors
- narrative/generator.py: run_narrative_stage accepts structural_anchors: Optional[StructuralAnchors] = None; TrailerManifest instantiation includes structural_anchors=structural_anchors kwarg
- Generated TRAILER_MANIFEST.json includes structural_anchors block (verified via model_dump_json test)
